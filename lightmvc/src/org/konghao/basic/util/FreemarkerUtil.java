...
public class FreemarkerUtil {
	...
	public static FreemarkerUtil getInstance(String pname) {
		if(util==null) {
			...
			cfg.setDefaultEncoding("UTF-8");//ybliu
	...
	public void fprint(Map<String,Object> root,String fname,String outpath) {
		try {
			 Writer out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outpath),"UTF-8"));//ybliu
			 //getTemplate(fname).process(root, new FileWriter(outpath));
			 getTemplate(fname).process(root, out);//ybliu
			 out.flush();//ybliu
		} catch (TemplateException e) {
			e.printStackTrace();
		} catch (IOException e) {
